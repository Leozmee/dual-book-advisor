"""
Gestionnaire Llama avec initialisation paresseuse
Fichier: agents/lazy_llama_manager.py

Version simplifiée qui évite les blocages au démarrage de Django
"""
import logging
import time
import json
...
class LazyLlamaManager:
    ...
    def __init__(self, base_url: str = "http://localhost:11434"):
        ...
        # Charger les statistiques (sans initialiser Ollama)
        self._load_stats()
        
        logger.info(f"✅ LazyLlamaManager créé (initialisation différée)")
    
    def _ensure_initialized(self):
        """Initialise le client Ollama seulement quand nécessaire"""
        if self._initialized:
            return
        
        try:
            # Import différé d'Ollama
            import ollama
            
            # Initialiser le client
            self.client = ollama.Client(host=self.base_url)
            
            # Vérifier la disponibilité (sans télécharger)
            try:
                ...
                if self.MODEL_NAME not in available_models:
                    logger.warning(f"⚠️ Modèle {self.MODEL_NAME} non disponible")
                    logger.info("💡 Installez-le avec: ollama pull llama3.2:3b")
                else:
                    logger.info(f"✅ Modèle {self.MODEL_NAME} disponible")
                
            except Exception as e:
                logger.warning(f"⚠️ Impossible de vérifier les modèles: {e}")
            
            self._initialized = True
            logger.info(f"✅ LazyLlamaManager initialisé")
            
        except ImportError:
            logger.error("❌ Ollama non installé. Installez avec: pip install ollama")
            raise
        except Exception as e:
...
Adapte ton style selon le type de contenu demandé."""
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if not self.is_available():
            recommendations.append("Service Ollama non disponible - Démarrez ollama serve")
...
lazy_llama_manager = LazyLlamaManager()